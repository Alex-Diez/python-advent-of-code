...
def find_bottom(programs):
    tree, reverse_tree = build_tower_and_reversed(programs)

    bottom = list(map(lambda e: e[0], filter(lambda e: e[1] == [], tree.items())))[0]
    while reverse_tree.get(bottom) is not None:
        bottom = reverse_tree.get(bottom)
    return bottom


def build_tower_and_reversed(programs):
    tree = dict()
    reverse_tree = dict()
    for prog in filter(lambda p: p.strip() is not '', programs.split('\n')):
        program = parse(prog)
        tree[program.name()] = program.sub_programs()
        for sub_program in program.sub_programs():
            reverse_tree[sub_program] = program.name()
...
def parse(program):
    ...
    if len(split) == 1:
        ...
        sub_programs_list = list(map(lambda e: e.strip(), split[1].split(', ')))
    name, weight = split[0].strip().split(' ')
    weight = weight.replace('(', '').replace(')', '')
    return Program(name, int(weight), sub_programs_list)


class Program(object):
    def __init__(self, name, weight, sub_programs):
        self._name = name
        self._weight = weight
        self._sub_programs = sub_programs
    ...
    def __repr__(self):
        return self.name() + '(' + str(self._weight) + ') -> ' + '[' + ', '.join(self.sub_programs()) + ']'


class ProgramsTowerTest(unittest.TestCase):
    def testTowerOfOneProgram(self):
        self.assertEqual('bottom', find_bottom('bottom (44)'))

    def testTowerOfTwoPrograms(self):
        self.assertEqual(
            'bottom',
            find_bottom(
                """
    ...
    def testTowerOfThreePrograms(self):
        self.assertEqual(
            'bottom',
            find_bottom(
                """
    ...
    def testPrintSolution(self):
        print(find_bottom(PUZZLE_INPUT))
