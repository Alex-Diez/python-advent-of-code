...
def contains_duplicates(phrases):
    ...
    return len(phrases_set) == 1


def has_anagrams(phrases):
    phrases_list = phrases.split(' ')
    length = len(phrases_list)
    for index in range(0, length):
        current_phrase = phrases_list[index]
        current_phrase_len = len(current_phrase)
        current_phrase_sorted = sorted(current_phrase)
        for next_index in range(index + 1, length):
            if current_phrase_len == len(phrases_list[next_index]) \
                    and current_phrase_sorted == sorted(phrases_list[next_index]):
                return True

    return False


def print_solution(validation_predicate):
    print(len(list(filter(validation_predicate, task_input.split('\n')))))


class SecurityParaphraseTest(unittest.TestCase):
    def testSingleParaphraseIsInValid(self):
        self.assertTrue(contains_duplicates('aa'))

    def testTwoSameParaphrase_areInvalid(self):
        self.assertTrue(contains_duplicates('aa aa'))

    def testTaskInput(self):
        self.assertFalse(contains_duplicates('aa bb cc dd ee'))
        self.assertTrue(contains_duplicates('aa bb cc dd aa'))
        self.assertFalse(contains_duplicates('aa bb cc dd aaa'))

    def testPrintSolution(self):
        print_solution(lambda line: not contains_duplicates(line))


class AnagramCheckTest(unittest.TestCase):
    def testSameSingleLetterString_areAnagrams(self):
        self.assertTrue(has_anagrams('a a'))

    def testDiffStrings_areNotAnagrams(self):
        self.assertFalse(has_anagrams('aa bb'))

    def testTaskInput(self):
        self.assertFalse(has_anagrams('abcde fghij'))
        self.assertTrue(has_anagrams('abcde xyz ecdab'))
        self.assertFalse(has_anagrams('a ab abc abd abf abj'))
        self.assertFalse(has_anagrams('iiii oiii ooii oooi oooo'))
        self.assertTrue(has_anagrams('oiii ioii iioi iiio'))

    def testPrintSolution(self):
        print_solution(lambda line: not (has_anagrams(line) or contains_duplicates(line)))
